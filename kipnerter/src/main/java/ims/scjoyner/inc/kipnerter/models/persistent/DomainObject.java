package ims.scjoyner.inc.kipnerter.models.persistent;
...
import ims.scjoyner.inc.kipnerter.utils.HibernateUtil;
...
public abstract class DomainObject <D extends DomainObject<D>> {
    ...
    /**
     * Saves a collection of domain objects in a single session
     *
     * @param collection
     *            of domain objects to save
     */
    public static void saveAll ( final Collection< ? extends DomainObject> collection ) {
        final Session session = HibernateUtil.openSession();
        session.beginTransaction();
        for ( final DomainObject object : collection ) {
